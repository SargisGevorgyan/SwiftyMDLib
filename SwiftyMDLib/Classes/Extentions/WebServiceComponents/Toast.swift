...
public extension UIView {
    ...
    func toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: ToastStyle) throws -> UIView {
        ...
        if let message = message {
            ...
            messageLabel?.awakeFromNib()
            
            let maxMessageSize = CGSize(width: self.bounds.size.width - imageRect.size.width - 48, height: self.bounds.size.height * style.maxHeightPercentage)
...
//        let longerX = max(titleRect.origin.x, messageRect.origin.x)
        let wrapperWidth = UIScreen.main.bounds.width - 32 //max((imageRect.size.width + (style.horizontalPadding * 2.0)), (longerX + longerWidth + style.horizontalPadding))
...
public enum ToastPosition {
    ...
    fileprivate func centerPoint(forToast toast: UIView, inSuperview superview: UIView) -> CGPoint {
        ...
        let bottomPadding: CGFloat = 54 + superview.csSafeAreaInsets.bottom
